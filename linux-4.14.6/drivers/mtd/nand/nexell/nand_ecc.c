...

{
	...
	int failed = 0, corrected = 0;

	int eccbytes = chip->ecc.bytes;
	int eccsize  = chip->ecc.size;
	int ecctotal = chip->ecc.total;// + 3;	// +3 Bad marker
	int eccpos   = mtd->oobsize - chip->ecc.total;
	...
	for ( n = eccpos ; num_steps; num_steps--, p += eccsize) {
		...
		if (NX_NAND_GetErrorStatus()) {
			...
			for (i = n-eccbytes; mtd->oobsize> i; i++) {
				if (0xff != chip->oob_poi[i])
					break;
			}
			if (mtd->oobsize == i)
				continue;
			...
			if (0 > ret) {
				...
				for (i=0 ; err > i; i++) {
					//printk(KERN_INFO "%d", e_pos[i]);
					//if (n != err-1)
						//printk(KERN_INFO ", ");

					if (4096 > e_pos[i]) {
						e_dat = (uint32_t*)p;
						e_dat[e_pos[i] / 32] ^= 1U<<(e_pos[i] % 32);
					}
					corrected++;
				}
				...
				uint8_t *buf, int page)
{
	...
	DBGOUT("%s, page=%d, ecc mode=%d, bytes=%d, mtd->writesize=%d\n", __func__, page, HW_ECC_MODE, eccbytes, mtd->writesize);

	if (512 >= mtd->writesize/*Spansion 1G 2048*/) {
		/* read oob */
	...
	for (n = eccpos; eccsteps; eccsteps--, p += eccsize) {
		for (i = 0; i < eccbytes; i++, n++) {
			ecc_code[i] = chip->oob_poi[n];
		}
		/* write ecc data to orignal ecc register */
...
#else
		for (i=0; i<1000;i++) {
			uint32_t reg = REG_NFECCSTATUS;
			printk("reg=%X\n", reg);
			if (reg & NX_NFECCSTATUS_DECDONE)
				break;
		}
#endif
		if (NX_NAND_GetErrorStatus()) {
			...
			if (0 > ret) {
				//ERROUT("nand detect ecc errs, can't correct (step=%d, page=%d, errcnt=%d ret=%d)\n",
				//	(chip->ecc.steps - eccsteps), page, err, ret);
				//ret = -EIO;
				failed++;
				//break;
			} else {
				//printk("nand detect ecc errs (step=%d, page=%d, errcnt=%d ret=%d) bit : ", (chip->ecc.steps - eccsteps), page, err, ret);
				  ...
				  const uint8_t *buf)
{
	...
	uint8_t	invalid_ecc[] = {0xe9, 0x1c, 0x9e, 0xd5, 0xb9, 0x92, 0x0b};
	...
	for (n = eccpos; eccsteps; eccsteps--, p += eccsize) {
		...
		/* check nand intialzie status */
		if (memcmp(ecc_code, invalid_ecc, eccbytes) == 0)
			memset(ecc_code, 0xff, eccbytes);
